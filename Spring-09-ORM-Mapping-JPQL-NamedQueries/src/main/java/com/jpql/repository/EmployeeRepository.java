...
public interface EmployeeRepository extends JpaRepository<Employee,Integer> {
    ...
    //single bind parameter, by sequence
    @Query("SELECT e FROM Employee e WHERE e.email = ?1 ")
    // Optional use when there is non-pointer exception happens, Ex, when there is no valid email
    ...
    // @Modifying @Transactional comes together when updating the query
    @Modifying
    @Transactional
